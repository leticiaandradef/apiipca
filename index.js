...
app.get('/historicoIPCA', (req, res) => {
    const { ano } = req.query;
    const dados = ano ? buscarIpcaPorAno(ano) : buscarIPCA();
...
app.get('/historicoIPCA/calculo', (req, res) => {
    ...
    if (!validaParametros(valorConvertido, mesInicio, anoInicio, mesFim, anoFim)) {
        return res.status(400).send({ erro: 'Os parâmetros fornecidos são inválidos.' });
    }

    const resultado = calculoIPCA(valorConvertido, mesInicio, anoInicio, mesFim, anoFim);
